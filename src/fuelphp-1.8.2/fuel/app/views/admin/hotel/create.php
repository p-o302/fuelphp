            ...
            <div class="card-body">
                <form action="/admin/hotel/store" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control <?= isset($errors['name']) ? 'is-invalid' : '' ?>" id="name" name="name" placeholder="Enter Hotel Name" value="<?= Input::post('name') ?>">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select <?= isset($errors['prefecture_id']) ? 'is-invalid' : '' ?>" id="prefecture_id" name="prefecture_id">
                            <option value="" disabled selected>Select Prefecture</option>
                            <?php foreach ($prefectures as $prefecture): ?>
                                <option value="<?= $prefecture->id ?>" <?= (Input::post('prefecture_id') == $prefecture->id) ? 'selected' : '' ?>><?= $prefecture->name_jp ?></option>
                            <?php endforeach; ?>
                        </select>
